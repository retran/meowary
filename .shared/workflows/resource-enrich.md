...
<done_when>Daily note checked; plan row read; article existence confirmed; skills loaded.</done_when>
</step>
...
<subagent_trigger>Spawn `url-fetcher` (`{{AGENT_DIR}}/agents/url-fetcher.md`) for web sources when count > 3 in 2f — pass URL, topic context, target `resources/sources/<article-slug>-<date>.md`; returns path + 3–5 facts. Spawn `confluence-fetcher` (`{{AGENT_DIR}}/agents/confluence-fetcher.md`) per Confluence page in 2b — pass URL/ID, matching article path, topic context; returns 3–7 facts + GDPR notes. Both parallel when count > 3; inline when ≤ 3.</subagent_trigger>
...
**7a Orphan scan:** `node {{AGENT_DIR}}/scripts/find-backlinks.js <article-path>`. Zero inbound = orphaned; fix by linking from closest related. (People exempt.)
