...
<subagent_trigger>For web URLs: spawn `url-fetcher` (`{{AGENT_DIR}}/agents/url-fetcher.md`) — pass URL, topic context, target `resources/sources/<slug>-<date>.md`; returns path + 3–5 facts + metadata. Inline (no agent) for Confluence/Jira/file (use respective skills/Read).</subagent_trigger>
