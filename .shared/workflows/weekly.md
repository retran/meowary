...
|-------|--------|----------|
| Weekly template | `{{AGENT_DIR}}/skills/journal/weekly-template.md` | If creating |
...
RUN `node {{AGENT_DIR}}/scripts/health-stale.js` to surface stale referenced articles.

UPDATE QMD index if new articles: `node {{AGENT_DIR}}/scripts/qmd-index.js --changed`.
...
| QMD index | `{{AGENT_DIR}}/index/` | Updated if changed |
