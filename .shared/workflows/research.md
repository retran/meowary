...
Run `qmd query "<topic>"` and `websearch "<topic>"` in parallel per context-gathering skill — see `{{AGENT_DIR}}/skills/context-gathering/` for patterns and query construction. For multi-source retrieval hierarchy (5 tiers: QMD → direct read → Confluence → Jira → web), also load `query` skill. Search `resources/` and web proactively. If question already answerable from existing knowledge: surface now and ask whether to continue.
...
2. Run QMD re-index if source notes added: `node {{AGENT_DIR}}/scripts/qmd-index.js --changed`.
...
4. DO NOT run `node {{AGENT_DIR}}/scripts/qmd-index.js` over flagged note. Must NOT enter index.
...
3. Update QMD index: `node {{AGENT_DIR}}/scripts/qmd-index.js --changed`.
