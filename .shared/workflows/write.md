...
- RFC → `{{AGENT_DIR}}/skills/projects/rfc-template.md`
- Spec → `{{AGENT_DIR}}/skills/projects/spec-template.md`
- ADR → see `design.md` (Step 6 ADR format block)
- Meeting notes → `{{AGENT_DIR}}/skills/journal/meeting-template.md`
...
Run `qmd query "<topic>"` and `websearch "<topic>"` in parallel per context-gathering skill — see `{{AGENT_DIR}}/skills/context-gathering/` for patterns and query construction. For prose quality (active voice, concision, formatting), also load `writing` skill. Search `resources/`, `projects/<name>/`, web proactively for relevant material before writing. Identify target audience: internal team, broader org, public?
