...
RUN `node {{AGENT_DIR}}/scripts/qmd-index.js --changed`.
...
- **`waiting-for.md` missing:** Create from `{{AGENT_DIR}}/meta-templates/waiting-for-template.md`. If template missing, create with `## Active`/`## Resolved` sections.
...
- **Weekly note missing on Friday:** Create from `{{AGENT_DIR}}/skills/journal/weekly-template.md` noting Monday skipped.
</error_handling>
