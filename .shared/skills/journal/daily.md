...
<creation_steps>
1. Copy `{{AGENT_DIR}}/skills/journal/daily-template.md`.
2. Replace placeholders: `{{DATE}}`, `{{DAY}}`, `{{WEEK_NUMBER}}`, `{{WEEK_FILE}}`, `{{PREV_DATE}}`, `{{NEXT_DATE}}`.
3. Populate `### Calendar` from `journal/recurring-events.md` (events for this weekday). If file missing, copy from `{{AGENT_DIR}}/meta-templates/recurring-events-template.md`.
4. VERIFY navigation links exist before writing.
</creation_steps>

<monday_planning>
On Monday, also perform weekly planning:
1. Check `journal/weekly/<year>-W<nn>.md`. Create from `{{AGENT_DIR}}/skills/journal/weekly-template.md` if missing.
...
- [ ] `journal/waiting-for.md` updated with new Waiting items? (If missing, copy from `{{AGENT_DIR}}/meta-templates/waiting-for-template.md`.)
</self_review>
