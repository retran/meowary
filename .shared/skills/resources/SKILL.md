...
All deterministic operations live in `{{AGENT_DIR}}/scripts/`.

### Find inbound links
```
node {{AGENT_DIR}}/scripts/find-backlinks.js <article-path>
```
USE after every rename, move, split, delete. Searches entire repo. Zero results = orphan.

### Health checks
```
node {{AGENT_DIR}}/scripts/health-all.js
```
...
```
node {{AGENT_DIR}}/scripts/report-resources.js [--sort actualized|lines|inlinks]
```
Table: path, lines, tags, actualized, inlinks, outlinks. USE before planning pass.

### Discover connections
```
node {{AGENT_DIR}}/scripts/discover-connections.js [--scope <path>] [--limit N]
```
...
<pre_check>Verify `meta/confluence-sync.json` exists. If missing, copy from `{{AGENT_DIR}}/meta-templates/confluence-sync-template.json`.</pre_check>
...
- Re-fetch when `node {{AGENT_DIR}}/scripts/confluence-updates.js YYYY-MM-DD` reports change.
...
<pre_check>Verify `meta/resources-log.md` exists. If missing, copy from `{{AGENT_DIR}}/meta-templates/resources-log-template.md`.</pre_check>
...
- Fix all inbound links after delete/merge/reclassify/split/rename: run `node {{AGENT_DIR}}/scripts/find-backlinks.js <old-path>`. Update or remove all matches. Broken links unacceptable.
</rules>
