...
**Input:** Operation type and details from `node .opencode/scripts/plan-resources.js` or a manually described operation.

To see all operations and their interactive prompts, run: `node .opencode/scripts/run-operation.js`

### Delete

1. Read the article to confirm it should be deleted.
2. Run `node .opencode/scripts/find-backlinks.js <article-path>` to find all inbound links.
...
5. Run `node .opencode/scripts/find-backlinks.js <absorbed-path>` to find all inbound links to the absorbed article. Redirect them to the surviving article.
...
7. Run `node .opencode/scripts/find-backlinks.js <source-path>` to find inbound links referencing the extracted content. Redirect to the new article where appropriate.
...
2. Run `node .opencode/scripts/health-orphans.js` to check for related orphaned articles that should be connected.
...
3. Run `node .opencode/scripts/find-backlinks.js <old-path>` to find all inbound links. Update them to the new path.
...
- **Fix all inbound links.** After any delete, merge, reclassify, split, or rename: run `node .opencode/scripts/find-backlinks.js <old-path>` to find every inbound link (not just in `resources/` — daily notes and project files link in too). Update or remove all matches. Broken links are unacceptable.
