...
2. **Update status.** In the project/area dashboard front matter: set `status: archived` and (for projects) also `status: Done` in the body. Set `updated` in front matter.
...
5. **Update context.md.** Remove the slug from the Active Projects or Active Areas list.

6. **Update tags.md.** If the project/area had a dedicated tag (e.g. `#p-<slug>`), keep it registered but note it as archived.

7. **Commit.** Commit all changes with message: "Archive <type> <slug>".
